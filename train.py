...
def train(model, train_data, epochs=5, learning_rate=0.01):
    for epoch in range(epochs):
        ...
        for i, (image, label) in enumerate(train_data):
            ...
            d_weights3 = np.dot(output_error.reshape(-1, 1), model.a2.reshape(1, -1))  # Shape (10, 16)
            d_biases3 = output_error
            
            hidden_error2 = np.dot(model.weights3.T, output_error) * model.a2 * (1 - model.a2)
            d_weights2 = np.dot(hidden_error2.reshape(-1, 1), model.a1.reshape(1, -1))  # Shape (16, 16)
